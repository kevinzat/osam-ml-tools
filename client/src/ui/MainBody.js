...
import KMeansClustersPanel from './KMeansClustersPanel';
...
const MODEL_SUFFIX = {
    ...
    'K-Means Clusters': ' Clusters (K-Means)'
  };
...
export default class MainBody extends React.Component {
  ...
  render() {
    if (this.state.show === 'table-loading') {
      ...
      } else if (model.type === 'K-Means Clusters') {
        return this.renderPanel(<div>
              <KMeansClustersPanel model={model}/>
            </div>);
      } else {
    ...
    return (
      <div className="container">
        ...
        <div className="row" style={{marginTop: '50px'}}>
          <div className="col-12">
            <center><a href="https://www.osam.com/Commentary/research_partners"
                className="small-link">OSAM Research Partners</a></center>
          </div>
        </div>
      </div>);
  }
  ...
  handleBuildModel(colIndex, modelType) {
    ...
    ScheduleLearn(tableName, colIndex, modelType,
        vals => {
          const name =
              ((colIndex === undefined) ? table.name : table.cols[colIndex].name) +
              MODEL_SUFFIX[modelType];
          const [params, trainErrs, testErrs] = vals;
          const features = table.cols.filter((v, index) => index !== colIndex);
          const model = new Model(name, features,
              (colIndex === undefined)
                  ? {name: 'Cluster', precision: 0, missing: 0, distinct: 10}
                  : table.cols[colIndex],
