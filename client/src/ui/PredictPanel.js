...
import { Closest } from '../learning/KMeans';
...
export default class PredictPanel extends React.Component {
  ...
  prediction(model, row, prob) {
    ...
    if (model.type === 'Linear Regression') {
      ...
      const k = (model.k !== undefined) ? model.k : 10;
      const [centers, scales] = model.params;
      return Closest(modelRow, centers[k-1], scales[k-1]) + 1;
