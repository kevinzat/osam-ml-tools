...
import { KMeansWrapper } from '../learning/KMeans';
 ...
 */
export function LearnColumn(table, colIndex, modelType, onProgress, args) {

  onProgress(5); // Show some progress right away..

  let exTrain, lblTrain, exTest, lblTest;
  if (colIndex !== undefined) {
    [exTrain, lblTrain] = GetData(table, colIndex, true);
    [exTest, lblTest] = GetData(table, colIndex, false);
  }
  ...
  } else if (modelType === 'K-Means Clusters') {
    let [centers, scales, fits] = KMeansWrapper(table, onProgress);
    return [[centers, scales], fits, undefined];
